...
\begin{frame}[fragile]
    \frametitle{\href{https://www.mailhardener.com/blog/why-mailhardener-recommends-spf-softfail-over-fail\#the-spf-bypass-vulnerability}{SPF bypass}}
    \foreignquote{english}{
        SPF uses the \texttt{rfc5321.MailFrom} address to determine the 'sender
        domain', which is where the SPF policy is fetched from.

        \vspace{0.3cm}

        However, the \texttt{rfc5321.MailFrom} address is not visible to the
        receiver, who only sees the \texttt{rfc5322.From} address as the
        sender. The flaw here is that the 'sender domain' and the domain used
        in the sender's email address do not have to match.
    }
...
\begin{frame}[fragile]
    \frametitle{\href{https://www.mailhardener.com/blog/spf-lookup-limit-explained\#the-spf-relaying-problem}{SPF relaying}}

    \foreignquote{english}{
        A relaying SMTP service relays (forwards) all incoming email to a
        different domain. Relaying services are common for organizations that
        have multiple domains.

        \vspace{0.3cm}
        For example: all emails from \texttt{old-company-name.com} may
        automatically be relayed to \texttt{new-company-name.com}.
    }

    \vspace{1cm}

    Après un relai, toutes les vérifications \texttt{SPF} échouent.
\end{frame}
